...
async function postToFacebook(message, imageUrl = null) {
  const appSecretProof = generateAppSecretProof(process.env.ZEUS_PAGE_TOKEN, process.env.ZEUS_APP_SECRET);
  let data = {
    message: message,
    access_token: process.env.ZEUS_PAGE_TOKEN,
    appsecret_proof: appSecretProof
  };
  if (imageUrl) {
    data.link = imageUrl; // For simplicity, use link; for photo upload, use multipart
  }
  const response = await axios.post(`https://graph.facebook.com/v18.0/${process.env.ZEUS_PAGE_ID}/feed`, data);
  return response.data;
}

// Create Instagram media
async function createInstagramMedia(caption, imageUrl) {
  const response = await axios.post(`https://graph.instagram.com/${process.env.APHRODITE_IG_ID}/media`, {
    image_url: imageUrl,
    caption: caption,
    access_token: process.env.APHRODITE_THREADS_TOKEN
  });
  return response.data;
}
...
async function publishInstagramMedia(creationId) {
  await axios.post(`https://graph.instagram.com/${process.env.APHRODITE_IG_ID}/media_publish`, {
...
async function createThreadsPost(text, imageUrl = null) {
  ...
  // Assume threadsUserId is APHRODITE_IG_ID or add separate env var
  const response = await axios.post(`https://graph.threads.net/v1.0/${process.env.APHRODITE_IG_ID}/threads`, data);
  return response.data;
}

// Publish Threads post
async function publishThreadsPost(creationId) {
  await axios.post(`https://graph.threads.net/v1.0/${process.env.APHRODITE_IG_ID}/threads_publish`, {
    creation_id: creationId,
    access_token: process.env.APHRODITE_THREADS_TOKEN
  });
}
